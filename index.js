...
const totalBackers = GAMES_JSON.reduce((acc, n) => acc + n.backers, 0);
...
const totalRaised = GAMES_JSON.reduce((acc, n) => acc + n.pledged, 0);
...
function filterUnfundedOnly() {
    ...
    let unfunded = GAMES_JSON.filter((g) => g.pledged < g.goal);
...
function filterFundedOnly() {
    ...
    let funded = GAMES_JSON.filter((g) => g.pledged >= g.goal);
...
const numUnfundedGames = GAMES_JSON.filter((g) => g.pledged < g.goal).length;

// create a string that explains the number of unfunded games using the ternary operator
let displayStr = `A total of $${totalRaised.toLocaleString('en-US')} has been raised for 
${GAMES_JSON.length.toLocaleString('en-US')} ${(GAMES_JSON.length > 1) ? "games" : "game"}. Currently,
 ${numUnfundedGames.toLocaleString('en-US')} ${(numUnfundedGames > 1) ? "games remain" : "game remains"} 
